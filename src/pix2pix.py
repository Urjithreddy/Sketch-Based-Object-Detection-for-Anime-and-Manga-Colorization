import sys
sys.path.append('d:/pragnya/capstone project/Manga-Colorizer-main/src')

import torch
from torchflare.experiments import Experiment
import torchflare.experiments
...
class Pix2PixExperiment(Experiment):
    ...
    def train_step(self):
        ...
        # Log the loss values
        print(f"train_G_loss: {G_loss.item()}, train_D_loss: {D_loss.item()}")

        return {"train_G_loss": G_loss.item(), "train_D_loss": D_loss.item()}
    ...
    def valid_step(self):
        ...
        # Log the loss values
        print(f"valid_G_loss: {G_loss.item()}, valid_D_loss: {D_loss.item()}")

        return {"valid_G_loss": G_loss.item(), "valid_D_loss": D_loss.item()}